...
Java_com_ziaee_frenchreader_llm_LlmNative_nativeLoadModel(
    ...
    try {
        modelPath = env->GetStringUTFChars(jModelPath, nullptr);
...
Java_com_ziaee_frenchreader_llm_LlmNative_nativeUnload(JNIEnv*, jobject, jlong handlePtr) {
    try {
        ...
        LOGE("nativeUnload: exception: %s", e.what());
    } catch (...) {
        LOGE("nativeUnload: unknown exception");
    }
}
...
Java_com_ziaee_frenchreader_llm_LlmNative_nativeGenerate(
    ...
    const char* systemPromptChars = nullptr;
    const char* userPromptChars = nullptr;
    try {
        systemPromptChars = env->GetStringUTFChars(jSystemPrompt, nullptr);
        userPromptChars = env->GetStringUTFChars(jUserPrompt, nullptr);
        ...
        try {
            ...
            sampler = common_sampler_init(handle->model, sparams);
            if (grammarChars) {
                env->ReleaseStringUTFChars(jGrammar, grammarChars);
                grammarChars = nullptr;
            }
        ...
        if (systemPromptChars) env->ReleaseStringUTFChars(jSystemPrompt, systemPromptChars);
        if (userPromptChars) env->ReleaseStringUTFChars(jUserPrompt, userPromptChars);
